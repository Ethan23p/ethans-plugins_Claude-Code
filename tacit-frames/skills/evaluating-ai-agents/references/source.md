...
Author(s): Mikaela Grace, Jeremy Hadfield, Rodrigo Olivares, Jiri De Jonghe (Anthropic)
Published: Jan 09, 2026
Captured: 2026-07-15
Capture method: fetched live via browser page-text extraction (WebFetch returns only a summary)
Verbatim except: site navigation, newsletter signup, and footer chrome removed; the three grader
comparison tables and the "approaches for understanding AI agent performance" table are rendered as
labeled lists because the source's HTML table structure does not survive text extraction — cell text
is unchanged; inline hyperlink targets are not preserved (link text is unchanged).
-->
