...
Author(s): Lance Martin, Gabe Cemaj, and Michael Cohen (Anthropic)
Published: as stated on the page, no explicit date shown in extracted text (page title: "Scaling Managed Agents: Decoupling the brain from the hands")
Captured: 2026-09-03
Capture method: fetched live via browser page-text extraction (get_page_text on the rendered article element)
Verbatim except: site navigation, "Get started with Claude Managed Agents by following our docs." call-to-action banner retained as it opens the extracted article body; no tables or footnotes present in the extracted text; inline hyperlink targets are not preserved (link text is unchanged).
-->
